        ...
        while i < len(text):
            if text[i : i + 2] == "\x1b[":
                ...
                if j < len(text):
                    ...
                    # Incomplete sequence, step over the escape character
        ...
        while i < len(params):
            ...
            if param == 0:
                ...
                if 0 <= color_index < len(self.color_palette):
                    ...
                    result.append(f'<span style="color: {color}">')
                    self.span_stack.append("color")
                i += 2  # step over the 5 and color index
...
def test_ansi_to_html():
    ...
    for i, (input_text, description) in enumerate(test_cases, 1):
        ...
        balance_status = "✅ BALANCED" if open_spans == close_spans else "❌ UNBALANCED"
